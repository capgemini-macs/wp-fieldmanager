...
/* global ajaxurl, FMtagBox, array_unique_noempty */
...
(function ($) {
  ...
  /**
	 ...
	 */
  window.fm_array_unique_noempty = function (array) {
    var out = []
  ...
  window.FMtagBox = {
    ...
    parseTags: function (el) {
      ...
      var taxbox = $(el).closest('.tagsdiv')

      var thetags = taxbox.find('.the-tags')

      var current_tags = thetags.val().split(tagDelimiter)
    ...
    quickClicks: function (el) {
      ...
      var tagchecklist = $('.tagchecklist', el)

      var id = $(el).attr('id')

      var current_tags; var disabled
      ...
      $.each(current_tags, function (key, val) {
        ...
        if (!disabled) {
          ...
          xbutton = $('<button type="button" id="' + id + '-check-num-' + key + '" class="ntdelbutton">' +
						...
						'<span class="screen-reader-text">' + window.FMtagsL10n.removeTerm + ' ' + listItem.html() + '</span>' +
						'</button>')
          ...
          xbutton.on('click keypress', function (e) {
            ...
            if (e.type === 'click' || e.keyCode === 13 || e.keyCode === 32) {
              ...
              FMtagBox.userAction = 'remove'
              FMtagBox.parseTags(this)
            }
          })
    ...
    flushTags: function (el, a, f) {
      var tagsval; var newtags; var text
      ...
      newtags = fm_array_unique_noempty(newtags.split(tagDelimiter)).join(tagDelimiter)
      tags.val(newtags)
      this.quickClicks(el)

      if (!a) { newtag.val('') }
      if (typeof (f) === 'undefined') { newtag.focus() }
    ...
    screenReadersMessage: function () {
      ...
      switch (this.userAction) {
        case 'remove':
          message = window.FMtagsL10n.termRemoved
          break

        case 'add':
          message = window.FMtagsL10n.termAdded
          break
    ...
    init: function () {
      ...
      $('.tagsdiv').each(function () {
        FMtagBox.quickClicks(this)
      })

      $('.tagadd', ajaxtag).click(function () {
        FMtagBox.userAction = 'add'
        FMtagBox.flushTags($(this).closest('.tagsdiv'))
      })
      ...
      $('#post').submit(function () {
        $('div.tagsdiv').each(function () {
          FMtagBox.flushTags(this, false, 1)
        })
      })
    }
  }
